...
| [persistence.md](./persistence.md)           | What the device persists: the storage keyspace, the per-channel policy record and why corruption throws, the per-key serialization the sign-once registry depends on, what a reinstall keeps, the channel index allocator, and the recovery model. |
