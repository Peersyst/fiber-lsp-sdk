...
| [session.md](./session.md)                   | The signer session client: the effects it is given, its states, the establishment and each way it fails, one request at a time, the heartbeat, the reconnect backoff, device-initiated requests, and what a listener may do.                    |
